...
if __name__ == '__main__':
    ...
    for bid, u, interior_mask in zip(building_ids, all_u, all_interior_mask):
        ...
        row = {
            'building_id': bid,
            'mean_temp': float(stats['mean_temp'].get()),
            'std_temp': float(stats['std_temp'].get()),
            'pct_above_18': float(stats['pct_above_18'].get()),
            'pct_below_15': float(stats['pct_below_15'].get()),
        }
        rows.append(row)
    ...
    plt.savefig('figures/task12_mean_temperature_distribution.png')
    plt.close()